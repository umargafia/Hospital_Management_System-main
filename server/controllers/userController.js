...
exports.registerDoctor = async (req, res, next) => {
  try {
    ...
    const spec = await Spec.findOne({ specId: req.body.specId });
    if (!spec) {
      return res.status(404).send('Specialization not found');
    }

    req.body.specialization = spec.specName;
      ...
      if (!user) {
        ...
        user.comparePassword(password).then((result) => {
          if (result) {
            ...
            getDeatils(user.role, user.email).then((result) => {
  ...
  Hospital.findOne({ name: req.body.name })
    .then((result) => {
      if (result.remCout == 0) {
        ...
        Hospital.updateOne(
          { name: req.body.name },
          { $set: { remCount: result.remCount - 1 } }
        )
          .then((result2) => {
            if (result2.acknowledged) {
              Hospital.updateOne(
                { name: req.body.name },
                { $pop: { availableRooms: -1 } }
              )
                .then((result3) => {
